"""Run improved paraphrase experiments on Modal."""
import modal
import os
import json

app = modal.App("cs224n-improved-para")
...
def run_paraphrase(fine_tune_mode: str, lr: float, epochs: int,
                   batch_size: int = 16, model_size: str = "gpt2"):
...
def main():
    experiments = {
        # 5 epochs, batch_size=16 to fit in timeout
        "para_full_1e5": run_paraphrase.spawn("full-model", 1e-5, 5, 16),
        "para_full_2e5": run_paraphrase.spawn("full-model", 2e-5, 5, 16),
        "para_lora": run_paraphrase.spawn("lora", 1e-4, 5, 16),
    }
    ...
    for key in ["para_full_1e5", "para_full_2e5", "para_lora"]:
        if key in results:
            ...
            print(f"  {key:20s} ({mode}, lr={lr})  Dev Acc = {final_acc}")
    print("=" * 60)

    with open("improved_para_results.json", "w") as f:
        json.dump({k: v.get("stdout", "")[:3000] for k, v in results.items()}, f, indent=2)
    print("Saved to improved_para_results.json")
