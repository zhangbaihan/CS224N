...
class GPT2Model(GPTPreTrainedModel):
  ...
  def hidden_state_to_token(self, hidden_state):
    ...
    """
    ### YOUR CODE HERE
    word_embedding_weights = self.word_embedding.weight
    logits = hidden_state @ word_embedding_weights.T
    return logits


  @classmethod
  def from_pretrained(cls, model='gpt2', d=768, l=12, num_heads=12,
                      use_lora=False, lora_r=8, lora_alpha=16.0, lora_dropout=0.0):
    ...
    our_model = GPT2Model(GPT2Config(
        ...
        intermediate_size=d*3, use_lora=use_lora, lora_r=lora_r,
        lora_alpha=lora_alpha, lora_dropout=lora_dropout)).eval()
